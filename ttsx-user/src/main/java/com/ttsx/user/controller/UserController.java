...
import com.ttsx.utils.Md5;
import com.ttsx.utils.QQ_util;
...
public class UserController {
    ...
    public Map login(Memberinfo memberinfo, HttpServletResponse response, HttpSession session){
        ...
        if( one!=null ){
            ...
            String token=  JWTUtils.creatToken(   m, Constant.EXPIRETIME);
            ...
            redisTemplate.expire(token,Constant.EXPIRETIME, TimeUnit.SECONDS);
            return map;
        }
        map.put("code",0);
    ...
    @RequestMapping("/saveOno")
    public Map saveOno( @RequestParam("ono") String ono
                        ){
        Map map = new HashMap();
        try {
            if (ono ==null||"".equals(ono)) {
                throw new RuntimeException("错误的ono参数");
            }
            Integer o = Integer.parseInt(ono);
